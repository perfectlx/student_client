<template>
  <div>
    <el-table :data="tableData" border show-header stripe style="width: 100%">
      ...
      <el-table-column prop="tid" label="教师编号" width="150">
      </el-table-column>
...
export default {
  methods: {
    ...
    deleteCourseTeacher(row) {
      const that = this;
      axios
        .post("http://localhost:10086/findCourseTeacherInfoWithTerm", row)
        .then(function (resp) {
          if (resp.data === true) {
            that.$message({
              showClose: true,
  ...
  watch: {
    ruleForm: {
      handler(newRuleForm, oldRuleForm) {
        ...
        axios
          .post(
            "http://localhost:10086/courseTeacher/findCourseTeacherInfoWithTerm",
            newRuleForm
          )
          .then(function (resp) {
